...
import { TEXT, type UserRole } from '@/constants'
...
/**
 * AdminSidebar - Navigation sidebar for admin layout
...
export function AdminSidebar({ isOpen, onClose }: AdminSidebarProps) {
  ...
  const visibleNavItems = ADMIN_NAVIGATION.filter((item) => {
    if (!item.allowedRoles || item.allowedRoles.length === 0) {
      return true
    }
    return role && item.allowedRoles.includes(role as UserRole)
  })

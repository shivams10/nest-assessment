...
export function ExamListPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useExams({ page: 1, limit: 20 }, false)
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-muted-foreground">Select an exam to begin</p>
      </div>
                ...
                <Button
                  ...
                  onClick={() => {
                    if (exam.submissionId) {
                      navigate(
                        ROUTES.CANDIDATE_EXAM_RESULT.replace(':submissionId', exam.submissionId)
                      )
                    }
                  }}
                >
